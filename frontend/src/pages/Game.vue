<template>
    ...
    <div class="mx-auto text-center flex flex-col items-center justify-center mb-20">
        <div class="card container mt-10 mb-10">
            <textarea ref="ansInput" class="textarea textarea-white " style="border-color: hsl(var(--bc) / 1)" placeholder="输入答案" v-model="ans"
                v-auto-expand></textarea>
            <button class="btn btn-outline mt-5" @click="submit" :disabled="loading">
        ...
        <transition-group name="list">
            <template v-for="(record, index) in records" v-key="index">
                <div class="container alert alert-error text-white mb-5 result" v-if="!record.passed">
                    <div>
                        <h2 class="font-bold">
                ...
                <div class="container alert alert-success text-white mb-5 result" v-else>
                    <div>
                        <h2 class="font-bold">
                            ...
                            答案正确！{{ record.points != undefined ? `您得到了 ${record.points} 分！` : "" }}
                        </h2>
                        <div class="mt-3" v-if="record.msg.length"><pre>{{ record.msg }}</pre></div>
                    </div>
                </div>
