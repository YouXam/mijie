...
![64卦图](https://files.chouhsing.org/64.png)

Midoria7 已经布置好了一个**先天六十四卦**阵。在这个阵法中，阿拉斯加的方位会受到阵法影响，呈现出特定的规律。

只要找到这个规律，你就可以预测阿拉斯加的方位，然后就可以抓到阿拉斯加了！

阿拉斯加的初始方位角是随机的，但之后的每一回合都和上一回合的方位角有关，遵循一定规律。
...
「一定是萨摩耶干的！这家伙来无影去无踪，我们上次追牠就追丢了！」Midoria7 恨恨地说。`
    ...
    checker: async (ans, ctx) => {
        ...
        ctx.msg(`阿拉斯加的方位角为 ${now_alpha.toFixed(3)}°，你的预测错误，阿拉斯加逃到了另一个方位！请你预测下一个方位角。`)
        return false
    }
} as Plugin<['方位角']>
